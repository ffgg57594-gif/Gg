...
function buildUpstreamPayload(body, searchParams) {
  ...
  const messages = messagesFromInput(source, searchParams);

  // Build a clean payload for the upstream chat function. Presenton sends a
  // full OpenAI streaming request (stream, tools, tool_choice, response_format,
  // n, ...) that a simple upstream chat function usually rejects with
  // "Invalid request format". We deliberately forward only messages + model
  // (plus a couple of benign sampling knobs) and force a non-streaming call so
  // we always get a single, predictable response to re-stream back to Presenton.
  const payload = {
    stream: false,
  ...
  if (source.temperature != null) payload.temperature = source.temperature;
  if (source.max_tokens != null) payload.max_tokens = source.max_tokens;
  if (source.max_completion_tokens != null) {
    payload.max_completion_tokens = source.max_completion_tokens;
  }
  if (source.top_p != null) payload.top_p = source.top_p;
