...
function upstreamAuthHeader(req, searchParams, body) {
  // Forward the key Presenton sends (Authorization: Bearer ... or api_key/body)
  // to the upstream, because a bring-your-own-key Gemini proxy needs it.
  const header =
    req.headers.authorization ||
    req.headers.Authorization ||
    searchParams.get('api_key') ||
    searchParams.get('key') ||
    (body && (body.api_key || body.key)) ||
    '';
  if (header) return String(header);
  return null;
}

async function callUpstream(payload, authHeader) {
...
async function handleChat(req, res, body, searchParams, stream) {
  ...
  let upstream;
  try {
    upstream = await callUpstream(payload, upstreamAuthHeader(req, searchParams, body));
  } catch (error) {
