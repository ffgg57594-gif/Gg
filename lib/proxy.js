...
function gatewayInfo() {
  ...
  return {
    ...
    base_url_for_presenton: '/api/v1',
    model: defaultModel,
    models,
    endpoints: {
      models: 'GET /api/v1/models',
      chat_post: 'POST /api/v1/chat/completions',
      chat_get: 'GET /api/v1/chat/completions?prompt=...',
    },
  };
}
...
function normalizeMessages(messages) {
  // The upstream chat function (a Gemini proxy) is picky about message shape.
  // Presenton sends OpenAI-style messages whose content may be arrays of parts
  // (text/reasoning blocks) or objects, plus tool_call / reasoning fields a
  // plain chat function rejects with "Invalid request format". Collapse every
  // message to role + plain-string content and drop everything else so the
  // upstream always receives a clean, standard payload.
...
function buildUpstreamPayload(body, searchParams) {
  ...
  const messages = normalizeMessages(messagesFromInput(source, searchParams));

  // Build a minimal payload for the upstream chat function. Presenton sends a
  // full OpenAI streaming request (stream, tools, tool_choice, response_format,
  // n, temperature, ...) that a simple upstream chat function usually rejects
  // with "Invalid request format". Forward ONLY model + messages (normalized to
  // plain strings) — no stream flag, no sampling knobs — so the smallest
  // possible OpenAI shape is what the upstream sees. We always make a
  // non-streaming call and re-stream the single response back to Presenton.
  return {
...
async function callUpstream(payload) {
  ...
  // The upstream BYOK Gemini proxy accepts a request with NO Authorization
  // header (the user's direct GET curl with no key returns is_byok responses),
  // but rejects a present-but-invalid Authorization with 400 "Invalid request
  // format". So do NOT forward Presenton's dummy API key. Only attach an
  // Authorization header when a dedicated UPSTREAM_API_KEY is configured.
...
async function handleRequest(req, res) {
  try {
    ...
    // The model list is public: presenton fetches it during "Check for available
    // models", and gating it behind PROXY_API_KEY (which presenton's UI key may
    // not match) makes the add/connect step fail with a 401. Exempt models so
    // the connection check always succeeds; chat remains protected when a key
    // is configured.
    const publicPath =
      pathname === '/' ||
      pathname === '/v1' ||
    ...
    sendJson(
      ...
      404,
      openaiError(
        `Unknown path ${pathname}. Use GET /api/v1/models or POST /api/v1/chat/completions.`,
        'invalid_request_error',
        'not_found',
        404,
      ),
