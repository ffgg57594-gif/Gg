...
function buildUpstreamPayload(body, searchParams) {
  ...
  // Build a minimal, clean payload for the upstream chat function. Presenton
  // sends a full OpenAI streaming request (stream, tools, tool_choice,
  // response_format, n, temperature, ...) that a simple upstream chat function
  // usually rejects with "Invalid request format". Forward only model + messages
  // (normalized to plain strings) and force a non-streaming call so we always
  // get a single, predictable response to re-stream back to Presenton.
  return {
    stream: false,
